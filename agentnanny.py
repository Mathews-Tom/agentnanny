...
def _deny_to_codex_rules(deny_patterns: list[str]) -> str:
    """Convert agentnanny deny patterns to Codex Starlark exec policy rules.
    ...
    """
    rules: list[str] = []
    rules.append("# Generated by agentnanny — do not edit manually")
    for pattern in deny_patterns:
        m = re.match(r'^Bash\((.+)\)$', pattern)
        if not m:
            continue
        glob_str = m.group(1)
        # Expand alternation: Bash(curl*|wget*) → two rules
        for segment in glob_str.split("|"):
            # Strip trailing glob wildcard for prefix matching
            prefix = segment.rstrip("*").rstrip()
            if prefix:
                rules.append(
                    f'prefix_rule(pattern=["{prefix}"], decision="forbidden",'
                    f' justification="blocked by agentnanny")'
                )
    return "\n".join(rules) + "\n"


def _allow_to_codex_rules(allow_patterns: list[str]) -> str:
    """Convert agentnanny allow patterns to Codex Starlark exec policy rules.

    Only Bash(...) patterns translate.
    """
    rules: list[str] = []
    for pattern in allow_patterns:
        m = re.match(r'^Bash\((.+)\)$', pattern)
        ...
        for segment in glob_str.split("|"):
            ...
            if prefix:
                rules.append(
                    f'prefix_rule(pattern=["{prefix}"], decision="allow",'
                    f' justification="allowed by agentnanny")'
                )
    return "\n".join(rules)
...
def _apply_codex_session(policy: dict, cfg: dict, scope_id: str):
    ...
    deny_rules = _deny_to_codex_rules(deny_patterns)
    if deny_rules.strip():
        rules_parts.append(deny_rules)
    allow_rules = _allow_to_codex_rules(allow_patterns)
